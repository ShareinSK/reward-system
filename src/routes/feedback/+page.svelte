	...
	async function submit(e: Event) {
		...
		try {
			...
			if (insertError) {
				...
				window.location.href = `mailto:feedback@herohabits.app?subject=${encodeURIComponent('Hero Habits feedback')}&body=${body}`;
				sent = true;
				return;
			}
			sent = true;
...
<section class="page">
	<header>
		...
		<h1>Feedback</h1>
		<p class="lede">
			We’re soft-launching Hero Habits with a community. Tell us what works, what’s confusing, and
