...
async function sendWebPush(
	...
	const vapidSubject = Deno.env.get('VAPID_SUBJECT') || 'mailto:feedback@herohabits.app';
...
async function sendEmail(to: string, subject: string, body: string) {
	...
	const from = Deno.env.get('RESEND_FROM') || 'Hero Habits <onboarding@herohabits.app>';
...
Deno.serve(async (req) => {
	...
	for (const t of trials ?? []) {
			...
			for (const sub of (subs ?? []) as PushSub[]) {
				try {
					await sendWebPush(sub, {
						title: template.subject || 'Hero Habits',
						body: template.body,
						url: '/billing/'
					});
				} catch {
		...
		if (email && profile.email_opt_in) {
			await sendEmail(email, template.subject || 'Hero Habits', template.body);
		}
